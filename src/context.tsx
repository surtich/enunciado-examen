...
export interface RoomContextProps {
  ...
  changeFeaturedState: (roomId: string, featured: boolean) => void;
}
...
class RoomProvider extends Component<{}, RoomProviderState> {
  ...
  changeFeaturedState = (roomId: string, featured: boolean) => {
    const rooms = this.state.rooms.map(room => {
      if (room.id !== roomId) {
        return room;
      }
    ...
    return (
      <RoomContext.Provider
        value={{
          ...
          changeFeaturedState: this.changeFeaturedState,
          rooms
        }}
      >
        {this.props.children}
